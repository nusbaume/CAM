...
! Public interfaces
  ...
  public :: wiso_kmol_land       ! kinetic effects for land evap (Brutsaert)
...
!      difrm = (/ 1._r8, 1._r8, 0.9757_r8, 0.9727_r8, 0.9856_r8, 0.9679_r8/) ! Merlivat 1978 (direct from paper)
      !RPF - set diffusivity ratio to 1 for const frac vars to cut out any kinetic fractionation
      difrm = (/ 1._r8, 1._r8, 0.9757_r8, 0.9727_r8, 0.9858_r8, 0.9679_r8 /) ! Merlivat 1978 + Schoenemann et. al., 2014
!      difrm = (/ 1._r8, 1._r8, 0.9757_r8, 0.9727_r8, 0.9727_r8, 0.9679_r8, 1._r8 /) ! Merlivat 1978 + Schoenemann et. al., 2014
...
! RPF: notes on constant frac - this array isn't used, so can set to anything!
  real(r8), parameter, dimension(pwtspec) :: &  ! liquid/vapour
...
!=======================================================================
  subroutine wiso_kmol_land(isp,rbot,zbot,ustar,z0,alpkn)
!-----------------------------------------------------------------------
!
! Purpose: compute kinetic modifier for drag coefficient (Merlivat & Jouzel)
!
...
! Author: David Noone <dcn@caltech.edu> - Mon Jun 30 14:05:38 MDT 2003
!
! Modified by Rich Fiorella 201120 to allow for variable roughness length
! over land.
!-----------------------------------------------------------------------
    use shr_kind_mod,  only: r8 => shr_kind_r8
    use shr_const_mod, only: shr_const_g, shr_const_karman
...
!---------------------------- Arguments --------------------------------
    ...
    real(r8), intent(in)  :: zbot  ! height of lowest level (m)
    real(r8), intent(in)  :: ustar ! Friction velocity (m/s)
    real(r8), intent(in)  :: z0    ! roughness length - rpf
!
    real(r8), intent(out) :: alpkn ! kinetic fractionation factor (1-kmol)

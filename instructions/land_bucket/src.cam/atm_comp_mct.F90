...
!================================================================================
  ...
  subroutine atm_init_mct( EClock, cdata_a, x2a_a, a2x_a, NLFilename )
    ...
    if (first_time) then
       ...
       if (StepNo == 0) then
          ...
          call atm_import( x2a_a%rattr, cam_in, cam_out ) !RPF-needed to use cam_out vars in atm_import_export
          call cam_run1 ( cam_in, cam_out )
          call atm_export( cam_out, a2x_a%rattr )
       else
          call atm_read_srfrest_mct( EClock, x2a_a, a2x_a )
          !call atm_import( x2a_a%rattr, cam_in, restart_init=.true. )
          call atm_import( x2a_a%rattr, cam_in, cam_out, restart_init=.true. ) !RPF
          call cam_run1 ( cam_in, cam_out )
       end if
 ...
 subroutine atm_run_mct( EClock, cdata_a, x2a_a, a2x_a)
    ...
    !call atm_import( x2a_a%rattr, cam_in )
    call atm_import( x2a_a%rattr, cam_in, cam_out ) !RPF-add cam_out so that it can be used in atm_import_export.

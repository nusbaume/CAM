...
!
    ...
    !Water tracers:
    !Add shallow and deep buckets, RPF 200818. added 17O on 200909
    type(var_desc_t) :: SbuckH_desc, Sbuck16_desc, SbuckD_desc, Sbuck18_desc, Sbuck17_desc, &
                        DbuckH_desc, Dbuck16_desc, DbuckD_desc, Dbuck18_desc, Dbuck17_desc 
  ...
  CONTAINS
    ...
    !-------------
    ! shallow bucket
    ierr = pio_def_var(File, 'SbuckH',  pio_double, hdimids, SbuckH_desc)
    ierr = pio_def_var(File, 'Sbuck16', pio_double, hdimids, Sbuck16_desc)
    ierr = pio_def_var(File, 'SbuckD',  pio_double, hdimids, SbuckD_desc)
    ierr = pio_def_var(File, 'Sbuck18', pio_double, hdimids, Sbuck18_desc)
    ierr = pio_def_var(File, 'Sbuck17', pio_double, hdimids, Sbuck17_desc)
    !-------------
    ierr = pio_def_var(File, 'DbuckH',  pio_double, hdimids, DbuckH_desc)
    ierr = pio_def_var(File, 'Dbuck16', pio_double, hdimids, Dbuck16_desc)
    ierr = pio_def_var(File, 'DbuckD',  pio_double, hdimids, DbuckD_desc)
    ierr = pio_def_var(File, 'Dbuck18', pio_double, hdimids, Dbuck18_desc)
    ierr = pio_def_var(File, 'Dbuck17', pio_double, hdimids, Dbuck17_desc)
    !-------------
  ...
  subroutine write_restart_physics (File, cam_in, cam_out, pbuf2d)
      ...
      !Shallow bucket:
       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%SbuckH(:ncol)
       end do
       call pio_write_darray(File, SbuckH_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%Sbuck16(:ncol)
       end do
       call pio_write_darray(File, Sbuck16_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%SbuckD(:ncol)
       end do
       call pio_write_darray(File, SbuckD_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%Sbuck18(:ncol)
       end do
       call pio_write_darray(File, Sbuck18_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%Sbuck17(:ncol)
       end do
       call pio_write_darray(File, Sbuck17_desc, iodesc, tmpfield, ierr)
      !-------------
      ! deep bucket: -RPF, 200818
       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%DbuckH(:ncol)
       end do
       call pio_write_darray(File, DbuckH_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%Dbuck16(:ncol)
       end do
       call pio_write_darray(File, Dbuck16_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%DbuckD(:ncol)
       end do
       call pio_write_darray(File, DbuckD_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%Dbuck18(:ncol)
       end do
       call pio_write_darray(File, Dbuck18_desc, iodesc, tmpfield, ierr)

       do i=begchunk,endchunk
         ncol = cam_out(i)%ncol
         tmpfield(:ncol, i) = cam_out(i)%Dbuck17(:ncol)
       end do
       call pio_write_darray(File, Dbuck17_desc, iodesc, tmpfield, ierr)
      !-------------
...
!#######################################################################
    ...
    subroutine read_restart_physics(File, cam_in, cam_out, pbuf2d)
     ...
     !-------------
     !Shallow bucket:
     ierr = pio_inq_varid(File, 'SbuckH', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%SbuckH(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'Sbuck16', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%Sbuck16(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'SbuckD', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%SbuckD(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'Sbuck18', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%Sbuck18(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'Sbuck17', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%Sbuck17(i) = tmpfield2(i, c)
       end do
     end do
     !-------------
     !Deep bucket: -RPF, 200818
     ierr = pio_inq_varid(File, 'DbuckH', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%DbuckH(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'Dbuck16', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%Dbuck16(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'DbuckD', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%DbuckD(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'Dbuck18', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%Dbuck18(i) = tmpfield2(i, c)
       end do
     end do

     ierr = pio_inq_varid(File, 'Dbuck17', vardesc)
     call pio_read_darray(File, vardesc, iodesc, tmpfield2, ierr)
     do c=begchunk,endchunk
       do i=1,pcols
         cam_out(c)%Dbuck17(i) = tmpfield2(i, c)
       end do
     end do

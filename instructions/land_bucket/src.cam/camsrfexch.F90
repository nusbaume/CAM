module camsrfexch
  ...
  type cam_out_t 
     ...
     !simple isotopic land bucket model: -RPF following JN
     !200818 - adding shallow and deep buckets. -RPF
     real(r8) :: SbuckH(pcols)      !bulk water bucket mass
     real(r8) :: Sbuck16(pcols)     !isotopic water bucket masses
     real(r8) :: SbuckD(pcols)
     real(r8) :: Sbuck18(pcols)
     real(r8) :: Sbuck17(pcols) 
     real(r8) :: DbuckH(pcols)      !bulk water bucket mass
     real(r8) :: Dbuck16(pcols)     !isotopic water bucket masses
     real(r8) :: DbuckD(pcols)
     real(r8) :: Dbuck18(pcols)
     real(r8) :: Dbuck17(pcols)
  ...
  type cam_in_t    
     ...
     real(r8) :: z0m(pcols)              ! RPF - roughness length 
...
!===============================================================================
  ...
  subroutine atm2hub_alloc( cam_out )
    ...
    use phys_grid        ,  only: get_ncols_p, get_rlat_p, get_rlon_p !RPF - for setting bucket initial conditions  
    ...
    ! LOCAL VARIABLES:
    integer :: c            ! chunk index
    integer :: ncols, i     ! RPF - bucket
    real(r8) :: wtlat, R    ! latitude/longitude place holders RPF bucket
    real(r8), parameter :: radtodeg = 180.0_r8/acos(-1.0_r8) !RPF - bucket
    integer :: ierror       ! Error code
    ...
    do c = begchunk,endchunk
       ...
       !initialize simple isotopic land bucket model -RPF following JN
       ncols = get_ncols_p(c)
       cam_out(c)%DbuckH(:)    = 0.300_r8 !initialize deep bucket as nearly 
       cam_out(c)%Dbuck16(:)   = 0.300_r8 !empty to speed up spinup.
       cam_out(c)%SbuckH(:)    = 0.040_r8 !initialize as glass half-full
       cam_out(c)%Sbuck16(:)   = 0.040_r8

       do i = 1,ncols
          
         wtlat = get_rlat_p(c,i)*radtodeg

         ! apply ratio to d2H buckets:
         R = 0.97623_r8 - 0.001976846_r8*abs(wtlat)

         cam_out(c)%DbuckD(i)    = R*cam_out(c)%Dbuck16(i) 
         cam_out(c)%SbuckD(i)    = R*cam_out(c)%Sbuck16(i)
 
         ! apply ratio to d18O buckets:
         R = 0.99607_r8 - 0.0002494177_r8*abs(wtlat)

         cam_out(c)%Dbuck18(i)   = R*cam_out(c)%Dbuck16(i) 
         cam_out(c)%Sbuck18(i)   = R*cam_out(c)%Sbuck16(i)

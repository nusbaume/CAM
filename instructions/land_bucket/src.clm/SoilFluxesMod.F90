...
!         t_veg                   => temperature_inst%t_veg_patch            , & ! Output: [real(r8) (:)   ]  vegetation temperature (Kelvin) 
         ...
         !RPF - evap_tot2 for evap_tot-lake_evap
         qflx_evap_tot2          => waterfluxbulk_inst%qflx_evap_tot2_patch     , & ! Output: [real(r8) (:)   ]  qflx_evap_soi + qflx_evap_veg + qflx_tran_veg - lake_evap
         !\RPF
      ...
      do fp = 1,num_nolakep
         ...
         qflx_evap_tot2(p)= qflx_evap_tot(p) !RPF - second version of evap_tot w/o lake fluxes

module WaterFluxType
  ...
  type, public :: waterflux_type
     ...
     !RPF - add second version of evap_tot w/out lake evaporation flux
     real(r8), pointer :: qflx_evap_tot2_patch     (:)   ! patch pft_qflx_evap_soi + pft_qflx_evap_veg + qflx_tran_veg - lake_evap
     real(r8), pointer :: qflx_evap_tot2_col       (:)   ! col col_qflx_evap_soi + col_qflx_evap_veg + qflx_tran_veg - lake_evap
     !\RPF
...
contains
  ...
  subroutine InitAllocate(this, bounds, tracer_vars)
    ...
    call AllocateVar1d(var = this%qflx_evap_tot_col, name = 'qflx_evap_tot_col', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_COLUMN)
    !RPF - evap_tot version w/o lake evap
    call AllocateVar1d(var = this%qflx_evap_tot2_col, name = 'qflx_evap_tot2_col', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_COLUMN)
    !\RPF
    ...
    call AllocateVar1d(var = this%qflx_evap_tot_patch, name = 'qflx_evap_tot_patch', &
         ...
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_PATCH)
    !RPF - evap_tot version w/o lake evap
    call AllocateVar1d(var = this%qflx_evap_tot2_patch, name = 'qflx_evap_tot2_patch', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_PATCH)
    !\RPF
  ...
  subroutine InitHistory(this, bounds)
    ...
    !RPF - adding second version w/ no lake evap
    this%qflx_evap_tot2_patch(begp:endp) = spval
    call hist_addfld1d ( &
         fname=this%info%fname('QFLX_EVAP_TOT_NOLAKE'), &
         units='mm H2O/s', &
         avgflag='A', &
         long_name=this%info%lname('qflx_evap_soi + qflx_evap_can + qflx_tran_veg - lake_evap'), &
         ptr_patch=this%qflx_evap_tot2_patch, c2l_scale_type='urbanf')
    !\RPF

...
contains
  ...
  subroutine InitHistory(this, bounds)
    ...
    !RPF test - trying to take slices of the soil water column. 200823.
    data2dptr => this%h2osoi_liq_col(begc:endc,1:3)
    call hist_addfld2d ( &
         fname=this%info%fname('TOPSOLLIQ'),  &
         units='kg/m2', type2d='levsoi', &
         avgflag='A', &
         long_name=this%info%lname('soil liquid water (vegetated landunits only)'), &
         ptr_col=data2dptr, l2g_scale_type='veg')

    data2dptr => this%h2osoi_liq_col(begc:endc,4:9)
    call hist_addfld2d ( &
         fname=this%info%fname('BOTSOLLIQ'),  &
         units='kg/m2', type2d='levsoi', &
         avgflag='A', &
         long_name=this%info%lname('soil liquid water (vegetated landunits only)'), &
         ptr_col=data2dptr, l2g_scale_type='veg')

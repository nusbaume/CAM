...
#include "shr_assert.h"
  ...
  type, public :: waterlnd2atm_type
     ...
     !RPF - trying to register qflx_evap_soi, qflx_evap_can, and qflx_tran_veg separately.
     real(r8), pointer :: qflx_evap_tot2_grc (:)   ! qflx_evap_soi + qflx_evap_can + qflx_tran_veg - lake evaporation
     real(r8), pointer :: qflx_evap_soi_grc  (:)   ! soil evaporation
     real(r8), pointer :: qflx_evap_can_grc  (:)   ! interception 
     real(r8), pointer :: qflx_tran_veg_grc  (:)   ! transpiration
     !/RPF
...
contains
  ...
  subroutine InitAllocate(this, bounds, tracer_vars)
    ...
    call AllocateVar1d(var = this%qflx_evap_tot_grc, name = 'qflx_evap_tot_grc', &
         ...
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_GRIDCELL, &
         ival=ival)
    !RPF-add ET constituent fluxes (and total flux - lake flux, added 201019)
    call AllocateVar1d(var = this%qflx_evap_tot2_grc, name = 'qflx_evap_tot2_grc', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_GRIDCELL, &
         ival=ival)
    call AllocateVar1d(var = this%qflx_evap_soi_grc, name = 'qflx_evap_soi_grc', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_GRIDCELL, &
         ival=ival)
    call AllocateVar1d(var = this%qflx_evap_can_grc, name = 'qflx_evap_can_grc', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_GRIDCELL, &
         ival=ival)
    call AllocateVar1d(var = this%qflx_tran_veg_grc, name = 'qflx_tran_veg_grc', &
         container = tracer_vars, &
         bounds = bounds, subgrid_level = BOUNDS_SUBGRID_GRIDCELL, &
         ival=ival)
    !/RPF

...
module seq_diag_mct
  ...
  !RPF - add constituent water fluxes.
  integer(in),parameter :: f_wtran     =16     ! water: transpiration
  integer(in),parameter :: f_wsevp     =17     ! water: soil (and lake?) evaporation
  integer(in),parameter :: f_wintr     =18     ! water: canopy interception
  !/RPF
  integer(in),parameter :: f_wsalt     =19     ! water: water equivalent of salt flux
  ...
  character(len=12),parameter :: fname(f_size) = &
       ...
       '       wevap','       wtran','       wsevp','       wintr',                &
       '    weqsaltf','     wrunoff','     wfrzrof',                               &
  ...
  !RPF - add submodel fluxes.
  integer :: index_x2a_Faxx_oevap
  integer :: index_x2a_Faxx_levap
  integer :: index_x2a_Faxx_trans !RPF
  integer :: index_x2a_Faxx_sevap !RPF
  integer :: index_x2a_Faxx_inter !RPF
  ...
  integer :: index_l2x_Fall_evpnl !RPF
  integer :: index_l2x_Fall_levap !RPF
  !RPF: not sure these three are necessary!
  integer :: index_l2x_Fall_trans 
  integer :: index_l2x_Fall_sevap
  integer :: index_l2x_Fall_inter !/RPF
  ...
  integer :: index_xao_Faox_oevap !RPF
  ...
  integer :: index_i2x_Faii_ievap !RPF
  ...
  subroutine seq_diag_atm_mct( atm, frac_a, infodata, do_a2x, do_x2a)
    ...
    if (present(do_x2a)) then
       if (first_time) then
          ...
          !RPF - not sure these are necessary here - adding but commenting out
          !since this routine I think only works with water budgets.
          !index_x2a_Faxx_trans  = mct_aVect_indexRA(x2a_a,'Faxx_trans')
          !index_x2a_Faxx_sevap  = mct_aVect_indexRA(x2a_a,'Faxx_sevap')
          !index_x2a_Faxx_inter  = mct_aVect_indexRA(x2a_a,'Faxx_inter')
       ...
       do n=1,lSize
          do k=1,4
             ...
             !RPF - same here, not sure these are needed in the budget calculations
             !nf = f_wtran; budg_dataL(nf,ic,ip) = budg_dataL(nf,ic,ip) + ca_a*x2a_a%rAttr(index_x2a_Faxx_trans,n)
             !nf = f_wsevp; budg_dataL(nf,ic,ip) = budg_dataL(nf,ic,ip) + ca_a*x2a_a%rAttr(index_x2a_Faxx_sevap,n)
             !nf = f_wintr; budg_dataL(nf,ic,ip) = budg_dataL(nf,ic,ip) + ca_a*x2a_a%rAttr(index_x2a_Faxx_inter,n)
             !/RPF
  ...
  subroutine seq_diag_lnd_mct( lnd, frac_l, infodata, do_l2x, do_x2l)
    ...
    if (present(do_l2x)) then
       if (first_time) then
          ...
          !RPF-same as subroutine above - not sure components are needed in budget calculations.
          !including, but commenting out.
          !index_l2x_Fall_trans  = mct_aVect_indexRA(l2x_l,'Fall_trans')
          !index_l2x_Fall_sevap  = mct_aVect_indexRA(l2x_l,'Fall_sevap')
          !index_l2x_Fall_inter  = mct_aVect_indexRA(l2x_l,'Fall_inter')
       ...
       do n=1,lSize
          ...
          !RPF - add constituents to budget?
          !nf = f_wevap ; budg_dataL(nf,ic,ip) = budg_dataL(nf,ic,ip) + ca_l*l2x_l%rAttr(index_l2x_Fall_evap,n)
          !nf = f_wevap ; budg_dataL(nf,ic,ip) = budg_dataL(nf,ic,ip) + ca_l*l2x_l%rAttr(index_l2x_Fall_evap,n)
          !nf = f_wevap ; budg_dataL(nf,ic,ip) = budg_dataL(nf,ic,ip) + ca_l*l2x_l%rAttr(index_l2x_Fall_evap,n)
